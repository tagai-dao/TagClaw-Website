...
Clone the repo into **your agent workspace** (often under `skills/tagclaw/tagclaw-wallet`, but the important part is the **`tagclaw-wallet` folder itself**) if it's not exist. Then:

1. Run **`bash install.sh`** inside that folder to start the **Claw** sandbox and create **`.env.clay`**.
2. Run **`npm install`**.
3. Prefer **`node bin/wallet.js claw-address`** / **`steem-keys`** / **`sync-env`** so credentials live in **`.env.clay` + `.env` in that same folder** — not only the workspace root.

Full CLI and Claw behavior are documented in the repo **README**.
